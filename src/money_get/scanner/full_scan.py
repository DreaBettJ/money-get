...
def full_scan(stock_count: int = 500, workers: int = 20) -> list:
    ...
    Returns:
        list: 排序后的结果
    """
    _logger.info(f"🚀 开始全市场扫描 (目标: {stock_count}只)")
    start_time = time.time()
    
    # 去重
    stocks = list(set(STOCK_POOL))[:stock_count]
    _logger.info(f"📋 股票池: {len(stocks)}只")
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            # 每50只输出进度
            if completed % 50 == 0:
                _logger.info(f"⏳ 进度: {completed}/{len(stocks)} (成功: {len(results)}, 失败: {fail_count})")
    ...
    _logger.info(f"✅ 扫描完成: {len(results)}只成功, {fail_count}只失败, 耗时 {elapsed:.1f}秒")
...
def format_report(results: list, top_n: int = 20) -> str:
    ...
    lines = []
    lines.append(f"\n{'='*70}")
    lines.append(f"📊 全市场扫描报告 - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    lines.append(f"{'='*70}")
    lines.append(f"扫描股票: {len(results)}只")
...
def run_full_scan(count: int = 500):
    """运行全市场扫描"""
    _logger.info(f"="*50)
    _logger.info(f"开始全市场扫描: {count}只股票")
    _logger.info(f"="*50)
    ...
    _logger.info(f"涨幅前5: {top5}")
    _logger.info(f"跌幅前5: {down5}")
